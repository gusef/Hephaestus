...
lo <- function(rown, 
  ...
  if(!is.null(coln[1]) | (!is.na2(annotation_row) & annotation_names_row)){
    ...
    tw = graphics::strwidth(t, units = 'in', cex = fontsize_col / fontsize)
    if(annotation_names_row){
      t = c(t, colnames(annotation_row))
      tw = c(tw, graphics::strwidth(colnames(annotation_row), units = 'in'))
    }
  ...
  if(!is.null(rown[1])){
    t = rown
    tw = graphics::strwidth(t, units = 'in', cex = fontsize_row / fontsize)
    if(annotation_names_col){
      t = c(t, colnames(annotation_col))
      tw = c(tw, graphics::strwidth(colnames(annotation_col), units = 'in'))
    }
  ...
  if(!is.na2(legend)){
    if (class(legend) == 'list'){
      nams <- unlist(lapply(legend, names))
    }else{
      nams <- names(legend)
    }
    longest_break = which.max(nchar(nams))
    longest_break = unit(1.1, "grobwidth", textGrob(as.character(nams)[longest_break], gp = do.call(gpar, gp)))
    title_length = unit(1.1, "grobwidth", textGrob("Scale", gp = gpar(fontface = "bold", ...)))
    legend_width = unit(12, "bigpts") + longest_break * 1.2
    if (class(legend) == 'list'){
      legend_width = legend_width + unit(12, "bigpts")
    }
  ...
  if(!is.null(barplot_legend)){
    ...
    # Column annotation height 
    barplot_leg_height = length(barplot_legend) * (textheight + unit(2, "bigpts")) + unit(2, "bigpts")
    
    # Width of the correponding legend
    t = c(barplot_legend_title, names(barplot_legend)) 
    
    barplot_leg_width = unit(1.2, "grobwidth", textGrob(t[which.max(nchar(t))], gp = gpar(...))) + unit(12, "bigpts")
  }
  else{
    barplot_leg_height = unit(0, "bigpts")
    barplot_leg_width = unit(0, "bigpts")
  }
  ...
  annot_legend_width = max(annot_row_legend_width, annot_col_legend_width, barplot_leg_width)
...
draw_barplot = function(bars, gaps, barplot_color, border_color, barplot_lim, barplot_censor){
  ...
  if(length(barplot_censor) == length(bars)){
    res[["text"]] = textGrob(x = coord_x, 
                             y = coord_y + unit(bars, "npc"),
                             just='bottom',
                             label = barplot_censor, 
                             gp = gpar(col = 'black', 
                                       fontsize = 8))
  }
...
draw_legend = function(color, breaks, legend, fontsize, ...){
  
  if (class(color) == 'list'){
    
    res <- gList()
    cur_height <- min(unit(0.9 / length(color), "npc"), unit(120 , "bigpts"))
    
    for (idx in 1:length(color)){
      br <- breaks[[idx]]
      leg <- legend[[idx]]
      
      legend_pos = ((leg - min(br)) * 0.9 / (max(br) - min(br)))
      legend_pos = cur_height * legend_pos + (unit(1, "npc") - cur_height * idx)
      
      br = (br - min(br)) * 0.9 / (max(br) - min(br))
      br = cur_height * br + (unit(1, "npc") - cur_height * idx)
      
      h = br[-1] - br[-length(br)]
      
      res[[paste0('rect',idx)]] <- rectGrob(x = unit(10, "bigpts"), 
                                            y = br[-length(br)] * 0.95, 
                                            width = unit(10, "bigpts"), 
                                            height = h, 
                                            hjust = 0, 
                                            vjust = 0, 
                                            gp = gpar(fill = color[[idx]]$color, col = "#FFFFFF00"))
      res[[paste0('text1_',idx)]] <- textGrob(names(color)[idx], 
                                              rot = 90,
                                              x = unit(3, "bigpts"), 
                                              y = legend_pos[ceiling(length(legend_pos) / 2)], 
                                              just = 'center',
                                              gp = gpar(fontsize = fontsize, fontface = 2))
      
      res[[paste0('text',idx)]] <- textGrob(names(leg), 
                                            x = unit(24, "bigpts"), 
                                            y = legend_pos * 0.95, 
                                            hjust = 0, 
                                            gp = gpar(...))
...
draw_barplot_axis = function(bars, barplot_label, barplot_lim, ...){
  
  height = min(unit(1, "npc"), unit(150, "bigpts"))
  
  legend <- grid.pretty(range(as.vector(bars)))
  legend_pos = ((legend - barplot_lim[1]) / (barplot_lim[2] - barplot_lim[1]))
...
draw_annotations = function(converted_annotations, border_color, gaps, fontsize, fontsize_number, number_color, colfmat, horizontal){
  ...
  if(horizontal){
    ...
    res = gList()
    res[["rect"]] = rectGrob(x = coord$x, y = coord$y, width = coord_x$size, height = unit(fontsize, "bigpts"), gp = gpar(fill = converted_annotations, col = border_color))
    if(attr(colfmat, "draw")){
      res[["text"]] = textGrob(x = coord$x, y = coord$y, label = t(colfmat), gp = gpar(col = number_color, fontsize = fontsize_number))
    }
    res = gTree(children = res)
...
heatmap_motor = function(matrix, 
  ...
  if(!is.na(filename)){
    ...
    f = switch(ending,
               ...
               jpeg = function(x, ...) grDevices::jpeg(x, units = "in", res = 300, ...),
               jpg = function(x, ...) grDevices::jpeg(x, units = "in", res = 300, ...),
               tiff = function(x, ...) grDevices::tiff(x, units = "in", res = 300, compression = "lzw", ...),
               bmp = function(x, ...) grDevices::bmp(x, units = "in", res = 300, ...),
  ...
  if(!is.na2(tree_col) & treeheight_col != 0){
    if (class(tree_col) == "numeric"){
      elem = draw_barplot(tree_col, gaps_col, barplot_color, border_color, barplot_lim, barplot_censor)
      res = gtable_add_grob(res, elem, t = 2, l = 3, name = "col_tree")
      
      l = ifelse(is.null(labels_row), 5, 4)
      elem = draw_barplot_axis(tree_col, barplot_label, barplot_lim)
  ...
  if(!is.na2(annotation_col)){
    ...
    elem = draw_annotations(converted_annotation, border_color, gaps_col,fontsize, fontsize_number, number_color, colfmat, horizontal = T)
  ...
  if(!is.null(annotation_col) & !is.na2(annotation_row)){
    # Draw tracks
    converted_annotation = convert_annotations(annotation_row, annotation_colors)
    elem = draw_annotations(converted_annotation, border_color, gaps_row,fontsize, fontsize_number, number_color, colfmat, horizontal = F)
...
scale_vec_colours <- function (x, col = grDevices::rainbow(10), breaks = NA, na_col){
  res <- col[as.numeric(cut(x, breaks = breaks, include.lowest = T))]
  res[is.na(res)] <- na_col
  return(res)
}

scale_colours = function(mat, col = grDevices::rainbow(10), breaks = NA, na_col){
  mat = as.matrix(mat)
...
cluster_mat = function(mat, distance, method, optimal_ord){
...
#' @param optimal_ord Should the optimal leaf ordering algorithm of the cba package be used for clustering? (default: TRUE)
...
#' @param na_as_X Should NA values be marked with an X? (Default: TRUE)
...
#' @param barplot_color Color of the top barplot (Default: grey) can be a color for each sample
#' @param barplot_height Height of the barplot in bigpts (default: 30)
#' @param barplot_lim Limit of the barplot (Default: NA - Automatically extracts the limit *1.05 and sets the lower bound to 0)
#' @param barplot_censor Censor vector if the barplot is used for survival data
#' @param barplot_label Label for the top barplot
#' @param barplot_decreasing_order Ordering of the barplot. Can be T / F / NA. For NA no ordering is applied
...
#' @param barplot_legend Legend for the barplot
#' @param barplot_legend_title Title for the barplot legend
#' 
#' @return 
...
#' @author Daniel Gusenleitner <gusef24@gmail.com>
#' 
#' @examples
#' # Create test matrix
...
#' # Draw heatmaps
#' g_heatmap(test)
#' g_heatmap(test, kmeans_k = 2)
#' g_heatmap(test, scale = "row", clustering_distance_rows = "correlation")
#' g_heatmap(test, color = colorRampPalette(c("navy", "white", "firebrick3"))(50))
#' g_heatmap(test, cluster_row = FALSE)
#' g_heatmap(test, legend = FALSE)
#' 
#' # Show text within cells
#' g_heatmap(test, display_numbers = TRUE)
#' g_heatmap(test, display_numbers = TRUE, number_format = "%.1e")
#' g_heatmap(test, display_numbers = matrix(ifelse(test > 5, "*", ""), nrow(test)))
#' g_heatmap(test, cluster_row = FALSE, legend_breaks = -1:4, 
#'     legend_labels = c("0", "1e-4", "1e-3", "1e-2", "1e-1", "1"))
#' 
#' # Fix cell sizes and save to file with correct size
#' g_heatmap(test, cellwidth = 15, cellheight = 12, main = "Example heatmap")
#' g_heatmap(test, cellwidth = 15, cellheight = 12, fontsize = 8, 
#'     filename = paste0(tempdir(),"/test.pdf"))
#' 
...
#' g_heatmap(test, annotation_col = annotation_col)
#' g_heatmap(test, annotation_col = annotation_col, annotation_legend = FALSE)
#' g_heatmap(test, annotation_col = annotation_col, annotation_row = annotation_row)
#' 
#' 
#' # Specify colors
#' ann_colors = list(
...
#' )
#' 
#' g_heatmap(test, annotation_col = annotation_col, annotation_colors = ann_colors, main = "Title")
#' g_heatmap(test, annotation_col = annotation_col, annotation_row = annotation_row, 
#'          annotation_colors = ann_colors)
#' g_heatmap(test, annotation_col = annotation_col, annotation_colors = ann_colors[2]) 
#' 
#' # Gaps in heatmaps
#' g_heatmap(test, annotation_col = annotation_col, cluster_rows = FALSE, gaps_row = c(10, 14))
#' g_heatmap(test, annotation_col = annotation_col, cluster_rows = FALSE, gaps_row = c(10, 14), 
#'          cutree_col = 2)
#' 
...
#' "", "", "Il10", "Il15", "Il1b")
#' 
#' g_heatmap(test, annotation_col = annotation_col, labels_row = labels_row)
#' 
#' # Specifying clustering from distance matrix
#' drows = dist(test, method = "minkowski")
#' dcols = dist(t(test), method = "minkowski")
#' g_heatmap(test, clustering_distance_rows = drows, clustering_distance_cols = dcols)
#' 
...
#' }
#' 
#' g_heatmap(test, clustering_callback = callback)
#' 
#' \dontrun{
...
g_heatmap = function(mat, 
  ...
  if(is.matrix(display_numbers) | is.data.frame(display_numbers)){
    ...
    if(display_numbers){
      ...
      fmat <- matrix(c('','X')[is.na(mat) + 1],ncol=ncol(mat))
      fmat_draw = TRUE
    } else {
  ...
  #If there are NA values in the column annotations, show them as X
  if (any(is.na(annotation_col)) || !na_as_X){
    colfmat = NA
    colfmat_draw = FALSE
  }else if (na_as_X){
    colfmat = t(matrix(c('','X')[is.na(annotation_col) + 1], ncol = ncol(annotation_col)))
    colfmat_draw = TRUE
  } 
  ...
  if((class(cluster_rows) == "hclust") || cluster_rows){
    if(class(cluster_rows) == "hclust"){
      ...
      tree_row = cluster_mat(mat, distance = clustering_distance_rows, method = clustering_method, optimal_ord)
  ...
  if (class(cluster_cols) == 'numeric'){
    ...
    if (!is.null(barplot_specific_order) || !is.na(barplot_decreasing_order)){
      if (!is.null(barplot_specific_order)){
        ord <- barplot_specific_order
      } else{
    ...
    if(class(cluster_cols) == "hclust"){
      ...
      tree_col = cluster_mat(t(mat), distance = clustering_distance_cols, method = clustering_method, optimal_ord)
      tree_col = clustering_callback(tree_col, t(mat))
    }
    mat = mat[, tree_col$order, drop = FALSE]
    fmat = fmat[, tree_col$order, drop = FALSE]
    if (colfmat_draw){
      colfmat = colfmat[, tree_col$order, drop = FALSE]
    }
  ...
  attr(colfmat, "draw") = colfmat_draw
  ...
  if(is.na2(breaks)){
    if (class(color) != 'list'){
      ...
      breaks <- lapply(color,function(x,mat)generate_breaks(mat[x$index,],length(x$color)),mat)
  ...
  if (legend & is.na2(legend_breaks)) {
    if (class(color) != 'list'){
      ...
      legend <- lapply(breaks, 
                       function(x){
                         y <- grid.pretty(range(as.vector(x)))
                         names(y) <- y
                         return(y)
                       })
    }
